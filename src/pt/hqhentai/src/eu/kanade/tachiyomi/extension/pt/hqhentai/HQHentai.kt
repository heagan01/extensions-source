package eu.kanade.tachiyomi.extension.pt.hqhentai
...
class HQHentai : ParsedHttpSource() {
    override val name = "HQ Hentai"
    override val baseUrl = "https://www.hqhentai.com.br"
    ...
    override fun popularMangaSelector() = "article.TPost"
    override fun latestUpdatesSelector() = "main.alinhamento:has(ul.content-pagination) article.TPost"
    ...
    override fun searchMangaFromElement(element: Element): SManga {
        return SManga.create().apply {
            ...
            title = element.select("a > h3.Title").text()
            thumbnail_url = element.select("a > div.Image > figure > img").imgAttr()
        }
    }
    ...
    override fun mangaDetailsParse(document: Document): SManga {
        return SManga.create().apply {
            title = document.select("div.titulo-single > h1").text()
            genre = document.select("ul.top-margin:contains(Tags) a[href*=/tag/]").eachText().joinToString()
            thumbnail_url = document.select("#img_1").imgAttr()
            status = SManga.COMPLETED
            update_strategy = UpdateStrategy.ONLY_FETCH_ONCE
        }
    }

    override fun chapterListSelector() = "main.alinhamento"
    ...
    override fun pageListParse(document: Document): List<Page> {
        return document.select("div.fotos > div.foto > img").mapIndexed { idx, img ->
